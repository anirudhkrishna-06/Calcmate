...
class TopicBandit:
    def __init__(self, topics):
        # Default: All weights = 1.0 (Neutral)
        self.weights = {topic: 1.0 for topic in topics}
        self.min_weight = 0.1
        self.max_weight = 10.0 # Increased cap to allow strong focus

    def select_topics(self, k=5):
        """Weighted Selection (Softmax-like)"""
        topics = list(self.weights.keys())
        weights = list(self.weights.values())
        return random.choices(topics, weights=weights, k=k)

    def update(self, topic, is_correct):
        """The Learning Step (Inverse Reward)"""
        current = self.weights[topic]
        if is_correct:
            # Decay (Mastery) -> Show less often
            self.weights[topic] = max(self.min_weight, current * 0.7)
        else:
            # Boost (Weakness) -> Show MORE often
            self.weights[topic] = min(self.max_weight, current * 1.5)

    def save_profile(self, filename="user_memory.json"):
        """Saves current weights to a file"""
        try:
            with open(filename, 'w') as f:
                json.dump(self.weights, f)
            print(f"💾 [MEMORY] Weak spots saved to '{filename}'")
        except Exception as e:
            print(f"❌ Error saving memory: {e}")

    def load_profile(self, filename="user_memory.json"):
        ...
        try:
            ...
            # Update only matches keys to avoid errors if dataset changes
...
# ---------------------------------------------------------
# 4. PRACTICE FORUM (The Controller)
# ---------------------------------------------------------
class PracticeForum:
    def __init__(self):
    ...
    def grade_submission(self, user_input, ground_truth_numeric):
        """
        Robust grading that handles raw numbers, equations, and messy input.
        """
        import re
        from sympy import sympify
        
        # 1. CLEANUP: Remove common clutter
        # Users might type "x = 50" or "50" or "$50" or "50km"
        clean_input = user_input.replace("$", "").replace("km", "").replace("%", "").strip()
        
        # 2. DIRECT NUMBER CHECK (The Fast Path)
        # If user just typed "600" or "0.75", verify it directly.
        try:
            # Extract all numbers from input
            numbers = re.findall(r"[-+]?\d*\.\d+|\d+", clean_input)
            if numbers:
                # Check if ANY number in the input matches the answer
                # This handles "The answer is 600" or "x=600" or "25, 15"
                for num in numbers:
                    if abs(float(num) - float(ground_truth_numeric)) < 0.1:
                        return True
        except:
            pass

        # 3. SYMBOLIC CHECK (The Logic Path)
        # If direct match failed, maybe they wrote a fraction "3/4" or expression "15*40"
        try:
            # Solve using the existing SymbolicSolver
    ...
    def visualize_weights(self):
        """Prints a bar chart of current weak spots"""
        print("\n🧠 CURRENT LEARNING PROFILE (Higher Bar = Weak Spot):")
        total_w = sum(self.bandit.weights.values())
        sorted_weights = sorted(self.bandit.weights.items(), key=lambda x: x[1], reverse=True)
        ...
        for t, w in sorted_weights:
            ...
            # Color code status
            if w > 1.5: status = "🔴 WEAK SPOT"
            elif w < 0.8: status = "🟢 MASTERED"
            else: status = "⚪ NEUTRAL"
                
            print(f"   {t.ljust(22)} |{bar} {w:.2f} ({prob:.1f}%) {status}")

    def run_interactive_loop(self):
        print("\n" + "="*60)
        print("🎓 CALCMATE ADAPTIVE TUTOR - INTERACTIVE MODE")
        print("="*60)

        # --- STEP 1: MEMORY CHECK ---
        print("Do you want to load your previous weak spots?")
        choice = input("Type 'y' to load, or 'n' to start fresh: ").lower().strip()
        
        if choice == 'y':
            self.bandit.load_profile()
        else:
            print("✨ Starting with a neutral profile.")

        batch_count = 1
        
        # --- STEP 2: THE SESSION LOOP ---
        while True:
            ...
            # A. Get Batch Size
            try:
                n_str = input("How many questions for this batch? (Default 3): ").strip()
                n_questions = int(n_str) if n_str else 3
            except:
                n_questions = 3

            # B. Select Topics (Weighted)
            print(f"⚙️ Generating {n_questions} questions based on your profile...")
            session_topics = self.bandit.select_topics(k=n_questions)
            ...
            for i, topic in enumerate(session_topics, 1):
                ...
                # USER INPUT
                user_input = input("   ✍️ Your Equation (e.g. x=50): ").strip()
                ...
                if is_correct:
                    ...
                    print(f"   ❌ INCORRECT. (Answer: {q_data['ground_truth_numeric']})")
                    # Show explanation or solution text
                    print(f"   💡 Solution Step: {q_data['solution_text']}")

            # D. Batch Report & Visualization
            print("\n" + "-"*60)
            print(f"📊 BATCH RESULT: {score}/{n_questions}")
            self.visualize_weights()
            print("-" * 60)
            
            # E. Continuation Decision
            cont = input("\nDo you want to continue to the next batch? (y/n): ").lower().strip()
            
            if cont == 'y':
                print("\n🔄 Adapting curriculum... The next batch will focus on your failed topics.")
                batch_count += 1
                time.sleep(1)
            else:
                # F. Exit & Save
                print("\n🛑 Ending Session.")
                save = input("Do you want to SAVE your weak spots for next time? (y/n): ").lower().strip()
                if save == 'y':
...
if __name__ == "__main__":
    ...
    print("2. Adaptive Practice Forum (Real Interactive Mode)")
