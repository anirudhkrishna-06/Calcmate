...
class PracticeForum:
    def __init__(self):
        ...
        # WE USE YOUR EXISTING SOLVER HERE!
        # This connects the new feature to your existing "Brain"
        self.solver = SymbolicSolver() 
    ...
    def grade_submission(self, user_equations, ground_truth_numeric):
        ...
        # We check all variables found (x, y, etc)
    ...
    def run_session(self):
        ...
        # 1. Agent picks topics based on your weak spots
        session_topics = self.bandit.select_topics(k=5)
        ...
        for i, topic in enumerate(session_topics, 1):
            ...
            print(f"\n[{i}/5] Topic: {topic.upper()}")
            print(f"Question: {q_data['q']}")
            
            # --- SIMULATION (For Demo Purposes) ---
            # We pretend the user is BAD at Algebra (always wrong)
            # and GOOD at everything else (always right).
            if topic == 'algebra':
                user_input = ["x = 0"] # Simulating a wrong answer
                print(f"   [USER] Input: x = 0 (Simulated Wrong Answer)")
            else:
                user_input = [f"x = {q_data['ground_truth_numeric']}"] # Simulating correct answer
            ...
            if is_correct:
                print("   [JUDGE] Result: CORRECT")
                score += 1
            else:
                print("   [JUDGE] Result: INCORRECT")
        ...
        print("="*60)
        print(f" Session Complete. Score: {score}/5")
        print("\n[MEMORY] UPDATED TOPIC WEIGHTS (Higher = Weaker Spot):")
        for t, w in self.bandit.weights.items():
            status = "WEAK SPOT" if w > 1.0 else "MASTERED"
            print(f"   - {t.ljust(20)}: {w:.2f} [{status}]")

if __name__ == "__main__":
    print("🚀 Select Mode:")
    print("1. Standard Pipeline Test (Original)")
    print("2. Adaptive Practice Forum (New Feature)")
    
    choice = input("Enter 1 or 2: ").strip()
    
    if choice == "1":
        # Run original pipeline test
