...
class PracticeForum:
    ...
    def visualize_dashboard(self):
        """Visualizes the Probability Distributions clearly in Text"""
        print("\n🧠 BAYESIAN KNOWLEDGE STATE (The AI's Model of You):")
        print(f"   {'Topic'.ljust(25)} | {'Mastery'.ljust(10)} | {'Confidence'.ljust(10)} | Status")
        print("-" * 75)
        
        topics = sorted(self.bandit.params.keys(), key=lambda t: self.bandit.get_stats(t)[0])
    ...
    def generate_graph(self):
        """Generates a Matplotlib graph of Alpha/Beta parameters """
        try:
            # Prepare Data
            topics = list(self.bandit.params.keys())
            clean_labels = [t.replace('_', ' ').title() for t in topics]
            alphas = [self.bandit.params[t]['alpha'] for t in topics]
            betas = [self.bandit.params[t]['beta'] for t in topics]
            ...
            # Green = Alpha (Success)
            p1 = plt.barh(y_pos, alphas, color='#4CAF50', label='Alpha (Success/Growth)', height=0.6)
            # Red = Beta (Failure) - Stacked on top of Alpha
            p2 = plt.barh(y_pos, betas, left=alphas, color='#F44336', label='Beta (Needs Work)', height=0.6)
            
            # Styling
            plt.yticks(y_pos, clean_labels)
            plt.xlabel('Bayesian Evidence Count (Total Samples)')
            plt.title('Reinforcement Learning Parameters (Alpha vs Beta)')
            plt.legend()
            plt.grid(axis='x', linestyle='--', alpha=0.7)
            
            # Show the graph
            print("\n📊 Opening Graph Window... (Close window to continue)")
            plt.tight_layout()
            plt.show()
    ...
    def run_interactive_loop(self):
        ...
        while True:
            ...
            preview = Counter(current_batch_topics)
